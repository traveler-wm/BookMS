...
public class LendDao {
    ...
    private final static String BOOK_LEND_SQL_TWO="UPDATE book_info SET state = state - 1 WHERE book_id = ? and state != 0";

    private final static String BOOK_LEND_SQL_Three="UPDATE reader_info SET balanced = balanced - ? WHERE reader_id = ? and balanced >= ?";

    private final static String LEND_LIST_SQL="SELECT * FROM lend_list";
    private final static String LEND_DELETE_SQL="delete from lend_list where sernum = ?";
    ...
    public int bookLendThree(float price,int readerId){
        return  jdbcTemplate.update(BOOK_LEND_SQL_Three,new Object[]{price,readerId,price});
    }
    public int deleteLend(int sernum){
        return jdbcTemplate.update(LEND_DELETE_SQL,new Object[]{sernum});
    }
